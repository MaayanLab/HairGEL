function main(exampleGene, tableName, sampleNames, tabSelector){
	// Add a space after tabSelector
	...
	d3.select(tabSelector + "#barplot-container svg").remove()

	var svg = d3.select(tabSelector + "#barplot-container").append("svg")
	...
	function fpkmTable(json) {
		d3.select(tabSelector + "#fpkm-table-gene").remove()
		var table = d3.select(tabSelector + "#fpkm-table").append("table")
	...
	function barPlot(json) {
		d3.select(tabSelector + "#title").remove();
		d3.select(tabSelector + "#x-axis").remove();
		d3.select(tabSelector + "#y-axis").remove();
		...
		// add error bars
		var errorBars = svg.selectAll(tabSelector + ".errorBar").data(data);
	...
	$(tabSelector + '#gene').bind("enterKey",function(e){
	...
	$(tabSelector + '#go-button').click(function(){
		var geneInput = $(tabSelector + '#gene').val();
		$.getJSON('genes.php', {gene: geneInput, table: tableName}, function(json){
			barPlot(json);
			fpkmTable(json);
		})
	...
	$(tabSelector + '#gene').keyup(function(e){
		if(e.keyCode == 13) {
			$(this).trigger("enterKey");
		}
	});

	$(tabSelector + '.example').click(function(){ // to show example when clicked
	...
	d3.select(tabSelector + "#save-button").on("click", function(){
		saveSvgAsPng(document.querySelector(tabSelector + "#fpkm-barplot"), "fpkm-barplot.png"); 
	});
